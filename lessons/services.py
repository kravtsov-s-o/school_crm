        ...
        try:
            with transaction.atomic():
                ...
                if entering:
                    ...
                    self.lesson.save()
                    logger.warning("lesson %s: no-op transition %s→%s",
                                   self.lesson.pk, old, self.status)
                    return
                ...
                self.lesson.save()
                logger.info("lesson %s: %s→%s applied", self.lesson.pk, old, self.status)
        except Exception:
            logger.exception("lesson %s: applying failed → %s", self.lesson.pk, self.status)
            raise
